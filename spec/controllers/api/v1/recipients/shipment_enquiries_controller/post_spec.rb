...
RSpec.describe "Enquiring Shipping", type: :request do
  describe "POST /api/v1/recipients/shipping_enquiries" do
    ...
    let!(:service_provider) { Marketplace::User.create_mocked_service_provider }

    context "when params represent a valid enquiry" do
      let(:shipping_request) { FactoryBot.create(:shipping_request, user_id: recipient.id) }
      let(:params) { { shipping_enquiry: { shipping_request_id: shipping_request.id, service_provider_id: service_provider.id } } }
    ...
    context "when params are missing any of the required attributes" do
      let(:params) { { shipping_enquiry: { service_provider_id: service_provider.id } } }

      it { expect(subject).to eq 400 }
    end

    context "when params point to objects that do not exist" do
      let(:params) { { shipping_enquiry: { shipping_request_id: -1, service_provider_id: service_provider.id } } }
